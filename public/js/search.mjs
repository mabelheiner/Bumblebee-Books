...
async function getBooks() {
  ...
  if (book_name == "") {
    ...
    const response = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q=${book_name}&download=epub&filter=ebooks&key=` +
    ...
    if (response.ok) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (results[i].volumeInfo.averageRating) {
          book_div.innerHTML +=
            `<a href="/"><img src=${results[i].volumeInfo.imageLinks.thumbnail} alt=${results[i].volumeInfo.title}><h2>` +
            results[i].volumeInfo.title +
            `</h2>` +
            `<i>` +
            results[i].volumeInfo.authors[0] +
            `</i><p>` +
            results[i].volumeInfo.averageRating +
            "★</p><p>" +
            results[i].volumeInfo.categories[0] +
            `</p></a>`;
        } else {
          book_div.innerHTML +=
            `<a href="/"><img src=${results[i].volumeInfo.imageLinks.thumbnail} alt=${results[i].volumeInfo.title}><h2>` +
...
async function getAuthor() {
  ...
  if (author_name == "") {
    ...
    message = "";
    const response = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q=inauthor:${author_name}&filter=ebooks&download=epub&key=` +
        google_books_api_key
    );
    if (response.ok) {
      let data = await response.json();
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (results[i].volumeInfo.averageRating) {
          book_div.innerHTML +=
            `<img src=${results[i].volumeInfo.imageLinks.thumbnail} alt=${results[i].volumeInfo.title}><h2>` +
            results[i].volumeInfo.title +
            `</h2>` +
            `<i>` +
            results[i].volumeInfo.authors[0] +
